...
class InsightFacePipeline {
  ...
  static Future<List<double>?> processImageFull(File imageFile, {bool skipDetection = false}) async {
    if (!_isInitialized) {
      await initialize();
    }
  ...
  static List<Rect> _parseFaceDetections(
    ...
    try {
      ...
      if (outputShape.length == 3) {
        ...
        // [num_boxes, values]
        detections =
            List<List<double>>.from(output.map((det) => List<double>.from(det)));
      } else {
      ...
      for (int i = 0; i < detections.length; i++) {
        ...
        bool hasInvalidValues = detection.any((val) => 
          val == null || val.isNaN || val.isInfinite
        );
        ...
        final clippedRight =
            math.max(0.0, math.min(right, imgWidth.toDouble()));
        final clippedBottom =
            math.max(0.0, math.min(bottom, imgHeight.toDouble()));
  ...
  static Future<bool> storeFaceEmbedding(String personId, File imageFile, {bool skipDetection = false}) async {
