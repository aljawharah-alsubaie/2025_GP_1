    ...
    try {
      ...
      final payload = {
        "input_data": [
          {
            "fields": [
              "red_mean", "green_mean", "blue_mean", "red_std",
              "green_std", "blue_std", "brightness", "contrast",
              "red_max", "green_max", "blue_max", "red_min",
              "green_min", "blue_min", "aspect_ratio", "red_range",
              "green_range", "blue_range", "color_variance"
            ],
            "values": [features]
          }
        ]
      ...
      // أرسل الطلب
      final response = await http.post(
        Uri.parse('$_mlUrl/ml/v4/deployments/$_deploymentId/predictions?version=2021-05-01'),
        headers: {
